...
있고 `require` 도 통과하지만 컴파일된 바이너리가 없다. 이 상태는 플러그인을 멈추지
않는다 — 상태 변경 잠금이 파일 기반 경로로 내려가고 state 저장은 계속된다. 대신
죽은 소유자 회수가 SQLite 트랜잭션 대신 파일 mutex 에 의존하므로 안전 여유가
좁아진다. `native-deps` 가 그 상태를 따로 잡아 `npm rebuild better-sqlite3` 을
가리킨다. `npm install` 은 이미 설치된 패키지의 install script 를 다시 돌리지
않는다. 어느 쪽이든 점검이 낸 hint 의 명령을 그대로 따른다.
