...
- 사용자에게 이미 활성인 Claude Code `/goal` 이 있고 그 네이티브 goal 루프만
  지켜보고 싶어 한다.
...
이 스크립트는 세션 범위 `prd.json` 을 만들거나 검증한다.
낡은 PRD 상태를 정리한다.
`progress.txt` 를 초기화한다.
Stop 훅이 읽는 Ralph 루프 상태를 쓴다.
성공하면 JSON 요약을 출력한다.
...
구현을 시작하지 않는다.
스크립트가 출력한 사유를 보고한다.
이 상태 없이는 Ralph 가 턴을 넘어 지속되지 않는다.

`CLAUDE_PLUGIN_ROOT` 는 Claude Code 가 마지막으로 실행한 훅의 플러그인을
가리킨다.
그 플러그인이 항상 이 플러그인은 아니다.
명령이 파일이 없다고 하면 바로 그 상황이다.
그 상황은 게이트를 건너뛸 사유가 아니다.
그때는 `.claude-plugin/plugin.json` 의 `"name"` 이 `"let-me-go-home"` 인
디렉토리를 찾는다.
그리고 그 아래 스크립트를 절대경로로 호출한다.
그 디렉토리로 `cd` 하지 않는다.
cwd 가 바뀌면 아래 `--project-dir` 이 없을 때 대상 저장소가 플러그인 저장소로
뒤바뀐다.

`--project-dir` 은 이 실행이 다룰 저장소다.
그 값이 `prd.json`·`progress.txt` 의 위치와 루프 상태의 `project_path` 를 정한다.
Stop 훅은 그 `project_path` 를 세션 cwd 와 정확히 비교한다.
그래서 두 값이 어긋나면 루프 강제가 조용히 멈춘다.
이때 오류도 경고도 나지 않는다.

- 값은 세션의 작업 디렉토리다.
  저장소 하위 디렉토리를 줘도 스크립트가 저장소 루트로 올린다.
- `$CLAUDE_PROJECT_DIR` 이 비어 있으면 그 자리에 세션의 작업 디렉토리 절대경로를
  직접 적는다.
  플래그를 빼지 않는다.
- git 저장소가 아니거나 없는 경로면 스크립트가 실패한다.
  그때는 위 fail-closed 를 따른다.
- 출력 JSON 의 `directory` 와 `directory_source` 로 실제 채택된 값을 확인한다.
  `directory` 가 대상 저장소가 아니면 거기서 멈춘다.
  그리고 보고한다.
...
그리고 그 경로를 `prompt_file` 로 알린다.
그 뒤 입력 파일을 지우는 것을 허용한다.
루프 상태의 프롬프트는 공백을 한 칸으로 합친 한 줄이다.
이터레이션마다 재주입되는 `Task:` 뒤에는 길이와 무관하게 `Full task text:` 경로
줄이 붙는다.
프롬프트가 길면 `Task:` 가 앞부분 발췌로 줄어든다.
그리고 `Task flags:` 줄이 더 붙는다.

위치 인자로 넘길 때는 `REFINE_CHECK_BEGIN` 과 `REFINE_CHECK_END` 사이 구간을 넣지
않는다.
그 구간을 뺀 나머지만 넘긴다.
블록 본문의 꺾쇠 플레이스홀더와 따옴표를 그대로 argv 에 싣지 않는다.
이 명령은 인자를 셸로 받기 때문이다.
`--prompt-file` 로 넘길 때는 블록을 빼지 않는다.
...
`.lmgh/state/sessions/{sessionId}/prd.json` 에 세션 범위로 둔다.
...
Deslop 옵트아웃: `{{PROMPT}}` 에 `--no-deslop` 이 있으면 리뷰 후 필수 deslop 패스를
통째로 건너뛴다.
이 플래그는 정리 패스가 그 실행의 범위 밖이라고 의도한 경우에만 쓴다.

Refine 검수 옵트인: `{{PROMPT}}` 에 `--refine-check` 가 있으면 Step 1 의 refine 검수
게이트가 발동한다.
호출자는 검수 절차 본문을 `REFINE_CHECK_BEGIN` 과 `REFINE_CHECK_END` 사이에 실어
보낸다.
ralph 는 그 내용을 해석하지 않는다.
통과 조건은 그 블록이 정한다.
통과 기록의 형식과 자리는 ralph 가 정한다.
그 기록은 `progress.txt` 에 쓰는 `refine-check: pass session=<sessionId>` 한 줄이다.
플래그가 있는데 블록이 없으면 fail-closed 로 멈춘다.

리뷰어 선택: Ralph 프롬프트에 `--critic=architect` 또는 `--critic=critic` 을 넘겨
그 실행의 완료 리뷰어를 고른다.
...
git 상태는 경고 신호일 뿐이다.
Ralph 가 story 를 `passes: true` 로 자동 정리하는 경우는 하나뿐이다.
그 경우는 PRD 에 설정된 관측 가능한 증거가 있고 모든 검사가 통과할 때다:
...
검사 타입은 `fileExists` / `fileContains`(워킹트리)와 `gitGrep`(특정 ref 의 내용)이다.
마지막 검사가 뜻하는 것은 "trunk 에 내용으로 확인됨"이지 PR 상태가 아니다.
Ralph 는 검사가 설정되지 않은 story 를 절대 자동 표시하지 않는다.
정리된 story 는 `architectVerified: false` 를 유지한다.
...
기준이 효력을 잃는 길은 증거를 보존하는 개정 경로뿐이다.
조용한 삭제로 기준의 효력을 없애지 않는다.
측정이 이미 반증한 기준을 "충족"시키는 방식으로도 없애지 않는다.

구현이 어떤 기준을 실증적으로 거짓임을 밝히면(예: 지시문의 개수가 틀림) 그 기준을 개정한다:

1. 반증된 기준을 측정된 수정값으로 교체하거나, 대체할 것이 없으면 폐기한다.
2. 개정을 story 의 `criterionAmendments` 대장에 기록한다.
   원래 기준 문구는 글자 그대로 보존한다 (다시 쓰거나 지우지 않는다).
   함께 기록할 항목은 아래와 같다:
   - `kind`: `"replaced"` 또는 `"superseded"`
   - `original`: 반증된 기준 원문 (개정을 기록하는 시점에 아직 활성인 기준이다)
...
3. 이후 완료 검사는 활성 기준만 검증한다.
   대장은 감사 흔적을 남긴다.
   리뷰어는 그 흔적으로 원래 기준이 왜 효력을 잃었는지 본다.

규칙:
- 경계 있는 증거, 이유, 주체, 타임스탬프가 빠진 개정은 무효다.
  그런 PRD 는 조용히 약해지지 않는다.
  그 PRD 는 읽는 시점에 fail-closed 한다.
- 아직 활성인 원본만 개정한다.
  원본은 한 번만 개정한다.
...
- 손으로 편집한 PRD 도 같은 불변식을 지킨다.
  모순된 대장(원본이 여전히 활성이거나, 원본을 두 번 개정한 대장)은 PRD 를 무효로
  만든다.
...
- 백그라운드 작업(서브에이전트·셸·Monitor 등)을 기다릴 때는 폴링·sleep 없이 턴을
  끝낸다.
  Stop 훅은 Claude Code 가 넘기는 진행 중 작업 목록으로 대기를 알아본다.
  그리고 이터레이션을 올리지 않는다.
  그 작업이 보고하면 새 턴에서 루프가 이어진다
- `[RALPH LOOP - WAITING]` 차단을 받았는데 그 작업을 기다리는 중이면 곧바로 턴을 다시
  끝낸다.
  그 작업과 무관한 남은 일이 있으면 그 일을 한다
- Claude Code `/goal` 이 대화에 나오면 그것을 네이티브 세션 루프의 인계·증거
  출처로만 다룬다.
  비결정적 경고 처리 대신 결정적 충돌 정책 `refuse`, `adopt_existing`, `artifact_only` 를 쓴다.
...
1. PRD Setup (첫 이터레이션만):
   a. Ralph 이어가기 맥락에 뜬 활성 PRD 파일을 확인한다.
      ...
      Ralph 는 시작 시 하위 호환을 위해 프로젝트 수준의 레거시 `prd.json` /
      `.lmgh/prd.json` 을 그쪽으로 복사할 수 있다.
   b. 레거시 PRD 가 없으면 시스템이 활성 PRD 경로에 scaffold 를 자동 생성해 둔 상태다.
   c. CRITICAL: scaffold 를 다듬는다.
      자동 생성된 PRD 는 일반적인 수용 기준("Implementation is complete" 등)을 갖는다.
      반드시 작업별 기준으로 교체한다:
      - 기본은 story 1개다.
      ...
      - 아래 둘(분할 트리거) 중 하나가 성립할 때만 쪼갠다.
        ...
        - 호출자가 분할을 정한 경우다.
          ...
          다시 합치지 않는다
          - 프롬프트가 분할을 지시한 경우
          - 호출자가 특정 story 를 원문 그대로 넣으라고 한 경우.
            그 story 는 따로 둔다
          - 활성 PRD 가 자동 생성 scaffold 가 아닌 경우.
            story 가 2개 이상이거나 수용 기준이 "Implementation is complete" 로
            시작하는 보일러플레이트가 아니면 호출자가 정한 분할로 다룬다
        - 되돌릴 수 없는 경계가 작업 중간에 있어 그 앞뒤를 따로 완료 판정할 필요가
   ...
   e. 선택적 company-context 호출: 이터레이션이 다음 story 를 고르기 전에
      `.claude/lmgh.jsonc` 와 `~/.config/claude-lmgh/config.jsonc` (프로젝트가 사용자
      설정을 덮는다)에서 `companyContext.tool` 을 확인한다.
      그 도구가 설정돼 있으면 요약 `query` 로 그 MCP 도구를 호출한다.
      그 요약은 현재 작업, PRD 상태, 다음 story 선택 단계, 바뀌었거나 건드릴 법한
      영역을 담는다.
   ...
   f. Refine 검수 게이트 (`{{PROMPT}}` 에 `--refine-check` 가 있을 때만 — 없으면 이 항목을 건너뛴다):
      - `REFINE_CHECK_BEGIN` 과 `REFINE_CHECK_END` 사이 절차를 그대로 수행한다.
        ralph 는 그 내용을 해석하지 않는다.
        무엇을 통과로 볼지는 그 블록이 정한다
      - 이 단계에서 `prd.json` 을 고치는 것은 c 의 refine 재작성이다.
        `criterionAmendments` 대상이 아니다
      - 블록이 서브에이전트를 띄우면 그 대기로 턴이 끊길 수 있다.
        다음 이터레이션에서는 이 항목을 처음부터 다시 돌리지 않는다.
        남은 부분을 마저 수행한다
      ...
      - 플래그가 있는데 블록이 없으면 그 사실을 보고한다.
        그리고 멈춘다.
        검수 없이 Step 2 로 가지 않는다

2. 다음 story 선택: 아래 조건이면 Step 1f 를 마저 수행한 뒤 이 단계로 온다.
   조건은 프롬프트에 `--refine-check` 가 있는데 `progress.txt` 에 현재 세션 id 와
   일치하는 `refine-check: pass session=<sessionId>` 줄이 없는 것이다.
   그다음 활성 PRD 파일을 읽는다.
   그리고 `passes: false` 인 것 중 우선순위가 가장 높은 story 를 고른다.
   그 story 가 현재 초점이다.
...
4. 현재 story 의 수용 기준 검증:
   ...
   c. 구현이 어떤 기준을 실증적으로 거짓임을 밝히면(측정이 그것을 반증하면) story 를
      ...
      그다음 남은 활성 기준 검증을 이어간다
   d. 활성 기준 중 충족되지도 개정되지도 않은 것이 있으면 계속 작업한다.
      story 를 완료로 표시하지 않는다

5. story 완료 표시:
   a. 모든 활성 수용 기준을 검증했으면 개정본에 묶인 완료 주장을 만든다:
      `passes: true` 로 한다.
      ...
      - `markStoryComplete`·`markStoryIncomplete`·`markStoryArchitectVerified` 는
        넘긴 문구를 덧붙인다.
        `prd.json` 을 직접 고칠 때도 기존 `notes` 를 통째로 바꾸지 않는다
...
6. PRD 완료 확인:
   a. 활성 PRD 파일을 읽는다.
      모든 story 가 `passes: true` 인지 확인한다 (검증 안 된 활성 기준이 남아 있지 않은지).
   b. 전부 완료가 아니면 Step 2(다음 story 선택)로 돌아간다
   c. 전부 완료면 Step 7(리뷰어 검증)로 간다

7. 리뷰어 검증 (수용 기준 대조):
   - 리뷰어는 기본 `let-me-go-home:architect` 이고 `--critic=critic` 이면
     `let-me-go-home:critic` 이다.
     둘 다 `sonnet` 으로 돈다.
     tier 선택은 없다.
   ...
   - 승인 시: 같은 턴에서 즉시 Step 7.5 로 간다.
     ...
     승인된 판정을 보고 체크포인트로 다루는 것은 예의상 멈춤 안티패턴이다.

7.5 필수 Deslop 패스 (Step 7 승인 뒤 조건 없이 실행한다. `{{PROMPT}}` 에 `--no-deslop` 이 있으면 예외):

- Skill 도구로 `ai-slop-cleaner` 스킬을 `Skill("let-me-go-home:ai-slop-cleaner")` 로
  호출한다.
  이 호출 대상은 에이전트가 아니라 Skill 이다.
  실수로 `Task(subagent_type="let-me-go-home:ai-slop-cleaner")` 로 부르지 않는다.
  이름이 비슷한 에이전트로 대체하지 않는다.
  현재 Ralph 세션에서 바뀐 파일만 대상으로 표준 모드(`--review` 아님)로 돌린다.
- 범위를 Ralph 가 바꾼 파일 집합으로 한정한다.
...
- 회귀가 실패하면 cleaner 변경을 되돌리거나 회귀를 고친다.
  그리고 통과할 때까지 검증 루프를 다시 돌린다.
...
- [ ] 원래 작업의 모든 요구를 충족한 상태다 (범위 축소 없음)
...
- [ ] progress.txt 에 작업 세부와 학습이 있다
- [ ] 선택된 리뷰어의 검증 결과가 통과다 (구체적 수용 기준 대조)
...
- 다중 저장소 워크스페이스의 앵커: 상위 디렉토리에 `.lmgh-workspace` 마커를 두면
  하위 저장소의 여러 세션이 하나의 `.lmgh/` 를 공유한다.
  해석 순서는 `LMGH_STATE_DIR > .lmgh-workspace > git > cwd` 다.
- 세션 id 출처: 시작 게이트는 `CLAUDE_CODE_SESSION_ID` 를 읽는다.
  Claude Code 가 셸 환경에 그 값을 설정한다.
  훅은 자기 stdin 페이로드의 `session_id` 를 읽는다.
  `LMGH_SESSION_ID` 는 override 일 뿐이다.
  Claude Code 는 그 값을 설정하지 않는다.
- 같은 워크스페이스에서 ralph 를 둘 돌리면 `prd.json` 에서 충돌한다.
  서로 다른 세션 ID 를 쓴다 (훅 페이로드의 session_id 는 이미 Claude Code 세션별로
  격리돼 있다).
