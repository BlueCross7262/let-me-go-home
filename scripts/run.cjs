 ...
 * every hook as a supervised child. Upstream also ran six trusted
 * UserPromptSubmit/PreToolUse/PostToolUse scripts inside a Worker, and
 * session-end.mjs in the foreground; this fork ships none of those eight
 * scripts, so both paths were unreachable and were removed.
 */
...
function isDebugHooksEnabled() {
  return process.env.LMGH_DEBUG_HOOKS === '1' ||
    process.env.LMGH_DEBUG === '1' ||
    process.env.LMGH_DEBUG === 'true';
}
...
function superviseGenericChild(targetPath, extraArgs) {
  ...
  const child = spawn(process.execPath, [targetPath, ...extraArgs], {
    ...
    env: {
      ...process.env,
      LMGH_SESSION_OWNER_PID: process.env.LMGH_SESSION_OWNER_PID || String(process.ppid),
    },
    windowsHide: true,
...
if (require.main === module) {
  ...
  if (target === '--generic-child-supervisor') {
    ...
    if (!resolution) {
      ...
      // Every hook this fork ships runs as a generic child. Upstream also had a
      // Worker path for six trusted UserPromptSubmit/PreToolUse/PostToolUse
      // scripts and a foreground path for session-end.mjs; none of those eight
      // scripts exist here, so both branches were unreachable and are gone.
