 ...
 *
 * Single authoritative entry point for resolving the .lmgh root directory in
 * hook scripts, respecting the LMGH_STATE_DIR environment variable.
 *
 * Delegates to getLmghRoot() from dist/lib/worktree-paths.js (the canonical
 ...
 *   - The namespace names come from ./namespace.mjs, a plain sibling file
 *     generated from src/constants/namespace.ts at build time. That is a
 *     static import within scripts/lib and adds no dist dependency, so the
 *     fallback still works when dist is missing.
...
import { STATE_DIR, WORKSPACE_MARKER } from './namespace.mjs';
...
function findWorkspaceRoot(directory) {
  ...
  while (true) {
    ...
    if (existsSync(join(cursor, WORKSPACE_MARKER))) return cursor;
...
function workspaceIdentifier(workspaceRoot) {
  try {
    const config = JSON.parse(readFileSync(join(workspaceRoot, WORKSPACE_MARKER), 'utf8'));
...
/**
 * Resolve the .lmgh root directory, respecting LMGH_STATE_DIR.
 *
 * @param {string} directory - Worktree root directory
 * @returns {Promise<string>} Absolute path to the .lmgh root
 */
export async function resolveLmghStateRoot(directory) {
  const pluginRoot = process.env.CLAUDE_PLUGIN_ROOT;
  if (pluginRoot) {
    const distPath = join(pluginRoot, 'dist', 'lib', 'worktree-paths.js');
    if (existsSync(distPath)) {
      const { getLmghRoot } = await import(pathToFileURL(distPath).href);
      return getLmghRoot(directory);
    }
  }
  ...
  const workspaceRoot = findWorkspaceRoot(directory);
  if (workspaceRoot && isSafeWorkspaceRoot(workspaceRoot)) return join(workspaceRoot, STATE_DIR);
  const gitRoot = probeGitRoot(directory);
  if (gitRoot) return join(gitRoot, STATE_DIR);
  const home = resolve(homedir());
  return join(home, STATE_DIR);
}
...
export async function resolveSessionStatePathsForHook(directory, stateName, sessionId) {
  ...
  const legacy = join(lmghRoot, 'state', `${normalizedName}.json`);
  if (!sessionId) {
    return { readPath: legacy, writePath: legacy };
  }
  const sessionScoped = join(lmghRoot, 'state', 'sessions', sessionId, `${normalizedName}.json`);
