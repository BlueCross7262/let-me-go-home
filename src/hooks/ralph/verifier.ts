 ...
 * 4. If architect approves -> truly complete, use /let-me-go-home:cancel to exit
...
import { resolveSessionStatePath, ensureSessionStateDir, getLmghRoot } from '../../lib/worktree-paths.js';
...
function getVerificationAgentStep(mode?: RalphCriticMode): string {
  switch (getCriticMode(mode)) {
   ...
   \`\`\`
   Task(subagent_type="let-me-go-home:critic", prompt="Critically review this task completion claim...")
   \`\`\``;
    default:
      return `1. **Spawn Architect Agent** for verification:
   \`\`\`
   Task(subagent_type="let-me-go-home:architect", prompt="Verify this task completion claim...")
   \`\`\``;
  }
}
...
3. **Based on ${criticLabel}'s response:**
   - If APPROVED: Output the exact correlated approval tag \`${approvalTag}\`, then run \`/${PLUGIN_NAME}:cancel\` to cleanly exit
...
4. After ${criticLabel} approves, run \`/${PLUGIN_NAME}:cancel\` to cleanly exit
