...
/**
 * Get the path to progress.txt in .lmgh subdirectory
 */
export function getLmghProgressPath(directory: string): string {
  return join(getLmghRoot(directory), PROGRESS_FILENAME);
}

/**
 * Find progress.txt in a directory (checks both root and .lmgh)
 */
export function findProgressPath(directory: string): string | null {
  ...
  const lmghPath = getLmghProgressPath(directory);
...
export function initProgress(directory: string): boolean {
  const lmghDir = getLmghRoot(directory);
  if (!existsSync(lmghDir)) {
    try {
  ...
  const progressPath = getLmghProgressPath(directory);
...
export function appendProgress(
  ...
  if (!progressPath) {
    ...
    progressPath = getLmghProgressPath(directory);
  }
