...
import { getLmghRoot, getSessionStateDir } from '../../lib/worktree-paths.js';
...
/**
 * Get the path to the prd.json in .lmgh subdirectory
 */
export function getLmghPrdPath(directory: string): string {
  return join(getLmghRoot(directory), PRD_FILENAME);
}
...
export function getLegacyStatePrdPath(directory: string): string {
  return join(getLmghRoot(directory), 'state', PRD_FILENAME);
}
...
export function findPrdPath(directory: string, sessionId?: string): string | null {
  ...
  const lmghPath = getLmghPrdPath(directory);
  if (existsSync(lmghPath)) {
    return lmghPath;
  }
...
export function writePrd(directory: string, prd: PRD, sessionId?: string, expectedRevision?: string): boolean {
  ...
  if (sessionId) {
    ...
    // prefer writing to an existing legacy location, or .lmgh by default.
    prdPath = findPrdPath(directory) ?? getLmghPrdPath(directory);
  }
...
6. If ALL stories are done, run \`/${PLUGIN_NAME}:cancel\` to cleanly exit ralph mode and clean up all state files
