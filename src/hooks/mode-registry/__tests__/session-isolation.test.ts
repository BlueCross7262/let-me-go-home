...
// The path assertions below spell the expected suffix with POSIX separators on
// purpose — that is the shape being asserted, and keeping the literal is what
// makes the test an independent oracle. The values under test come from join(),
// so they carry the platform separator; normalize only that.
...
describe('Session-Scoped State Isolation', () => {
  ...
  function createSessionState(sessionId: string, mode: string, data: Record<string, unknown>) {
    const sessionDir = join(tempDir, '.lmgh', 'state', 'sessions', sessionId);
  ...
  function createLegacyState(mode: string, data: Record<string, unknown>) {
    const stateDir = join(tempDir, '.lmgh', 'state');
  ...
  describe('resolveSessionStatePath', () => {
    it('should return session-scoped path', () => {
      ...
      expect(posix(path)).toContain('.lmgh/state/sessions/session-123/ralph-state.json');
    });
  ...
  describe('Session-scoped path resolution', () => {
    it('should return session-scoped path when sessionId provided', () => {
      ...
      expect(posix(path)).toContain('sessions/session-123');
    });
  ...
  describe('Two sessions writing independent state', () => {
    it('should isolate state between sessions', () => {
      ...
      const pathA = join(tempDir, '.lmgh', 'state', 'sessions', 'session-A', 'ralph-state.json');
      const pathB = join(tempDir, '.lmgh', 'state', 'sessions', 'session-B', 'ralph-state.json');
  ...
  describe('clearModeState with sessionId', () => {
    ...
    it('should clear session-scoped marker artifacts (ralph verification) for the target session only', () => {
      ...
      const sessionADir = join(tempDir, '.lmgh', 'state', 'sessions', sessionA);
      const sessionBDir = join(tempDir, '.lmgh', 'state', 'sessions', sessionB);
      const markerA = join(sessionADir, 'ralph-verification-state.json');
      const markerB = join(sessionBDir, 'ralph-verification-state.json');
      const legacyMarker = join(tempDir, '.lmgh', 'state', 'ralph-verification.json');
      writeFileSync(markerA, JSON.stringify({ pending: true }, null, 2));
      writeFileSync(markerB, JSON.stringify({ pending: true }, null, 2));
      mkdirSync(join(tempDir, '.lmgh', 'state'), { recursive: true });
    ...
    it('should NOT delete legacy marker file owned by a different session', () => {
      ...
      const legacyMarkerDir = join(tempDir, '.lmgh', 'state');
    ...
    it('preserves a replacement marker created after ownership discovery', () => {
      const sessionA = 'session-A';
      const markerDir = join(tempDir, '.lmgh', 'state');
      ...
      process.env.LMGH_TEST_CONDITIONAL_CLEAR_REPLACEMENT_PATH = markerPath;
  ...
  describe('Stale session cleanup', () => {
    it('serializes marker writers on the same lock used by cleanup', () => {
      ...
      const markerPath = join(tempDir, '.lmgh', 'state', 'ralph-verification.json');
    ...
    it('should remove empty session directories', () => {
      const emptyDir = join(tempDir, '.lmgh', 'state', 'sessions', 'empty-session');
