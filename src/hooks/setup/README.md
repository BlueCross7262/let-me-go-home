...
Handles let-me-go-home initialization and maintenance tasks.

## Triggers

### `init`
Initializes let-me-go-home directory structure and environment on first run or explicit setup.

**What it does:**
- Creates required directories: `.lmgh/state/`, `.lmgh/logs/`, `.lmgh/notepads/`, `.lmgh/state/checkpoints/`, `.lmgh/plans/`
- Validates existing config files (`.lmgh-config.json`)
- Sets environment variables (`LMGH_INITIALIZED=true`) if `CLAUDE_ENV_FILE` is available
...
```json
{
  ...
  "hookSpecificOutput": {
    "hookEventName": "Setup",
    "additionalContext": "let-me-go-home initialized:\n- 5 directories created\n- 1 configs validated\n- Environment variables set: LMGH_INITIALIZED"
  }
}
```

### `maintenance`
Performs periodic maintenance tasks to keep let-me-go-home state clean.
...
Creates all required let-me-go-home directories.

**Returns:** Array of created directory paths.

```typescript
const created = ensureDirectoryStructure('/path/to/project');
// => ['/path/to/project/.lmgh/state', '/path/to/project/.lmgh/logs', ...]
```
...
// => ['/path/to/project/.lmgh-config.json']
```

### Environment Variables

#### `setEnvironmentVariables(): string[]`
Sets environment variables for let-me-go-home initialization.
