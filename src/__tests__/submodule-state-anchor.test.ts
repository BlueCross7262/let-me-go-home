 ...
 * returns the submodule's own root (a submodule is a complete git repo), so this plugin
 * created a stray `.lmgh/` inside the submodule working tree. The fix climbs to
...
import { getWorktreeRoot, getGitTopLevel, getLmghRoot, resolveToWorktreeRoot, clearWorktreeCache } from '../lib/worktree-paths.js';
...
describe('submodule state anchoring (issue #3349)', () => {
  ...
  beforeAll(() => {
    tempDir = realpathSync(mkdtempSync(join(tmpdir(), 'lmgh-submodule-')));
  ...
  it('getLmghRoot from inside a submodule anchors .lmgh/ to the superproject root', () => {
    if (!gitAvailable) return;
    clearWorktreeCache();
    const prev = process.env.LMGH_STATE_DIR;
    delete process.env.LMGH_STATE_DIR;
    try {
      expect(getLmghRoot(submodulePath)).toBe(join(superRoot, '.lmgh'));
    } finally {
      if (prev !== undefined) process.env.LMGH_STATE_DIR = prev;
      clearWorktreeCache();
    }
  });
  ...
  // resolving state. Under LMGH_STATE_DIR the resolved root becomes the
  ...
  it('resolveToWorktreeRoot preserves submodule identity under LMGH_STATE_DIR (no climb)', () => {
    if (!gitAvailable) return;
    clearWorktreeCache();
    const prev = process.env.LMGH_STATE_DIR;
    process.env.LMGH_STATE_DIR = join(tempDir, 'central');
    try {
      // LMGH_STATE_DIR set: identity-bearing root stays at the submodule.
  ...
  it('centralized .lmgh for a submodule session does not merge into the parent superproject', () => {
    if (!gitAvailable) return;
    clearWorktreeCache();
    const prev = process.env.LMGH_STATE_DIR;
    ...
    try {
      // Mirror the hook path: resolveToWorktreeRoot() then getLmghRoot().
      const submoduleLmgh = getLmghRoot(resolveToWorktreeRoot(submodulePath));
      clearWorktreeCache();
      const superLmgh = getLmghRoot(resolveToWorktreeRoot(superRoot));
      expect(submoduleLmgh).not.toBe(superLmgh);
      expect(submoduleLmgh.startsWith(central)).toBe(true);
    } finally {
      if (prev === undefined) delete process.env.LMGH_STATE_DIR;
      else process.env.LMGH_STATE_DIR = prev;
      clearWorktreeCache();
