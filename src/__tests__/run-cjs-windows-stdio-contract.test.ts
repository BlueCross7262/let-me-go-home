...
async function runDeclaredOrphan(declaredSec: number): Promise<{
  ...
  const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-budget-'));
  ...
  const runner = spawn(process.execPath, [RUN_CJS_PATH, target], {
    ...
    env: { ...process.env, LMGH_TEST_PIDFILE: pidfile, CLAUDE_PLUGIN_ROOT: pluginRoot },
    windowsHide: true,
  });
...
describe('run.cjs Windows/protocol stdio contract (#3920)', () => {
  ...
  it('keeps every shipped hook visible to the short-budget contract', () => {
    const manifest = JSON.parse(readFileSync(join(process.cwd(), 'hooks', 'hooks.json'), 'utf8')) as {
      hooks: Record<string, Array<{ hooks?: Array<{ command?: string; timeout?: number }> }>>;
    };
    const shipped = Object.values(manifest.hooks)
      .flatMap(groups => groups)
      .flatMap(group => group.hooks ?? []);
    expect(shipped.length).toBeGreaterThan(0);
    for (const hook of shipped) {
      const name = [...(hook.command ?? '').matchAll(/scripts[/\\]([^"\s]+)/g)].at(-1)?.[1];
      expect(name, `hook command resolves to a script: ${hook.command}`).toBeTruthy();
      expect(existsSync(join(process.cwd(), 'scripts', name as string))).toBe(true);
      const declaredMs = (hook.timeout ?? 60) * 1000;
      const inner = runCjs.resolveGenericTimeoutMs({ timeoutMs: declaredMs, event: 'Stop' }, 'win32');
      expect(inner, `inner budget for ${name}`).toBeGreaterThan(runCjs.NESTED_OPERATION_TIMEOUT_MS);
      expect(inner / declaredMs).toBeGreaterThanOrEqual(runCjs.MIN_HOOK_INNER_FRACTION);
    }
  });
  it('fail-opens inside a 1s declared budget without requiring cold-start completion', () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-one-second-'));
    try {
      const pluginRoot = join(directory, 'plugin');
  ...
  it('closes protocol stdout when the runner times out even if a detached descendant still lives', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-eof-'));
    const pidfile = join(directory, 'orphan.pid');
    let orphanPid: number | undefined;
    const pluginRoot = join(directory, 'plugin');
    const declaredSec = 3;
  ...
  it('closes protocol stdout after a successful hook exit even if a detached descendant still lives', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-success-eof-'));
    const pidfile = join(directory, 'orphan.pid');
    ...
    try {
      ...
      const result = spawnSync(process.execPath, [RUN_CJS_PATH, SUCCESS_ORPHAN], {
        ...
        env: { ...process.env, LMGH_TEST_PIDFILE: pidfile },
        windowsHide: true,
      });
  ...
  it('forwards hook stdout and stderr exactly once through the runner', () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-forward-'));
    try {
  ...
  it('forwards all 512KiB from a successful hook to a 1KiB/10ms consumer', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-slow-consumer-'));
  ...
  it.skipIf(process.platform === 'win32')('exits nonzero when a successful hook leaves output queued to a paused consumer', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-paused-success-'));
    try {
      const pluginRoot = join(directory, 'plugin');
...
process.exit(0);`,
  ...
  it('fail-opens when the protocol stdout consumer closes early', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-closed-out-'));
    let runner: ReturnType<typeof spawn> | undefined;
    try {
...
writeFileSync(process.env.LMGH_READY_FILE, 'ready');
const writeOutput = () => {
  if (!existsSync(process.env.LMGH_GO_FILE)) return setTimeout(writeOutput, 1);
...
`);
      const child = runner = spawn(process.execPath, [RUN_CJS_PATH, fixture], {
        stdio: ['ignore', 'pipe', 'pipe'],
        env: { ...process.env, LMGH_READY_FILE: readyFile, LMGH_GO_FILE: goFile },
        windowsHide: true,
      });
      let stderr = '';
  ...
  it('fail-opens when the protocol stderr consumer closes early', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-closed-err-'));
    try {
      const fixture = join(directory, 'write-hook.cjs');
      writeFileSync(fixture, 'process.stdout.write("OUT-BYTES"); process.stderr.write("ERR-BYTES");');
  ...
  it('bounds cleanup when a hook survives the closed stdout channel', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-closed-out-survivor-'));
    let runner: ReturnType<typeof spawn> | undefined;
    try {
      const readyFile = join(directory, 'hook-ready');
      const goFile = join(directory, 'hook-go');
...
writeFileSync(process.env.LMGH_READY_FILE, 'ready');
const start = () => {
  if (!existsSync(process.env.LMGH_GO_FILE)) return setTimeout(start, 1);
  process.stdout.write('OUT-BYTES');
  setInterval(() => {}, 1e9);
};
start();
`, 5);
      ...
      const child = runner = spawn(process.execPath, [RUN_CJS_PATH, target], {
        stdio: ['ignore', 'pipe', 'pipe'],
        env: { ...process.env, CLAUDE_PLUGIN_ROOT: pluginRoot, LMGH_READY_FILE: readyFile, LMGH_GO_FILE: goFile },
        windowsHide: true,
      });
      child.stderr!.resume();
      await waitForFile(readyFile);
  ...
  it('fail-opens a drain-aware noisy stdout hook when the consumer closes early', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-drain-out-'));
    try {
      const pluginRoot = join(directory, 'plugin');
...
`, 5);
  ...
  it('fail-opens a drain-aware noisy stderr hook when the consumer closes early', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-drain-err-'));
    try {
      const pluginRoot = join(directory, 'plugin');
...
`, 5);
  ...
  it('reaps grandchildren when stdout closes and the hook leader EPIPE-exits', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-close-reap-'));
  ...
  it('writes a visible timeout diagnostic on fail-open', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-timeout-diag-'));
    try {
      const pluginRoot = join(directory, 'plugin');
      const target = writePluginHook(pluginRoot, 'hang-hook.cjs', 'setInterval(() => {}, 1e9);', 1);
      const startedAt = Date.now();
  ...
  it('fail-opens a timed-out hook when the protocol stderr consumer is closed', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-timeout-closed-err-'));
    try {
      const pluginRoot = join(directory, 'plugin');
      const target = writePluginHook(pluginRoot, 'hang-hook.cjs', 'setInterval(() => {}, 1e9);', 1);
      const startedAt = Date.now();
  ...
  it('fail-opens when a paused stderr consumer closes after the timeout diagnostic is queued', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-paused-err-'));
    try {
      const pluginRoot = join(directory, 'plugin');
      const target = writePluginHook(pluginRoot, 'hang-hook.cjs', 'setInterval(() => {}, 1e9);', 1);
      const startedAt = Date.now();
  ...
  it('exits on timeout when a noisy hook has a non-draining protocol consumer', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-noisy-'));
    let runner: ReturnType<typeof spawn> | undefined;
    try {
      const pluginRoot = join(directory, 'plugin');
      const target = writePluginHook(
  ...
  it('reaps grandchildren when the hook leader exits on protocol EPIPE at timeout', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-epipe-reap-'));
    const pidfile = join(directory, 'grandchild.pid');
    const previousPidfile = process.env.LMGH_TEST_PIDFILE;
    let grandchildPid: number | undefined;
    process.env.LMGH_TEST_PIDFILE = pidfile;
    const innerMs = 1500;
    const outerMs = 3000;
    try {
      const startedAt = Date.now();
      ...
      if (previousPidfile === undefined) delete process.env.LMGH_TEST_PIDFILE;
      else process.env.LMGH_TEST_PIDFILE = previousPidfile;
      killIfAlive(grandchildPid);
      rmSync(directory, { recursive: true, force: true });
    }
  });
  ...
  it('reaps timed-out generic descendants so no orphan survives', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'lmgh-stdio-reap-'));
    const pidfile = join(directory, 'grandchild.pid');
    const previousPidfile = process.env.LMGH_TEST_PIDFILE;
    let grandchildPid: number | undefined;
    process.env.LMGH_TEST_PIDFILE = pidfile;
