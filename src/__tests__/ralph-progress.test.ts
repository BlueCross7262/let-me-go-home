...
describe('Ralph Progress Module', () => {
  ...
  describe('initProgress', () => {
    it('should create progress.txt in .lmgh directory', () => {
      expect(initProgress(testDir)).toBe(true);
      expect(existsSync(join(testDir, '.lmgh', PROGRESS_FILENAME))).toBe(true);
    });
  ...
  describe('readProgressRaw / readProgress', () => {
    ...
    it('should read progress from .lmgh directory', () => {
      const lmghDir = join(testDir, '.lmgh');
      mkdirSync(lmghDir, { recursive: true });
      writeFileSync(join(lmghDir, PROGRESS_FILENAME), '# Test');
      expect(readProgressRaw(testDir)).toBe('# Test');
    });
  });
