...
describe('Ralph PRD Module', () => {
  ...
  describe('findPrdPath', () => {
    ...
    it('should find prd.json in .lmgh directory', () => {
      const lmghDir = join(testDir, '.lmgh');
      mkdirSync(lmghDir, { recursive: true });
      const prdPath = join(lmghDir, PRD_FILENAME);
      writeFileSync(prdPath, '{}');
      expect(findPrdPath(testDir)).toBe(prdPath);
    });

    it('should prefer root over .lmgh', () => {
      const rootPath = join(testDir, PRD_FILENAME);
      const lmghDir = join(testDir, '.lmgh');
      mkdirSync(lmghDir, { recursive: true });
      const lmghPath = join(lmghDir, PRD_FILENAME);

      writeFileSync(rootPath, '{"source": "root"}');
      writeFileSync(lmghPath, '{"source": "lmgh"}');
  ...
  describe('readPrd / writePrd', () => {
    ...
    it('should create .lmgh directory when writing', () => {
      writePrd(testDir, samplePrd);
      expect(existsSync(join(testDir, '.lmgh'))).toBe(true);
    });
    ...
    it('isolates transient PRDs for concurrent sessions in the same project', () => {
      ...
      expect(existsSync(join(testDir, '.lmgh', 'prd.json'))).toBe(false);
    });
    ...
    it('migrates an existing project PRD into the requesting session without mutating the legacy file', () => {
      ...
      const legacyPath = join(testDir, '.lmgh', 'prd.json');
      mkdirSync(join(testDir, '.lmgh'), { recursive: true });
