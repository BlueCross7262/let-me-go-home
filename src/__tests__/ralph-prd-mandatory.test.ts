...
describe('Ralph PRD-Mandatory', () => {
  ...
  beforeEach(() => {
    ...
    // Create .lmgh/state directory for ralph state files
    mkdirSync(join(testDir, '.lmgh', 'state'), { recursive: true });
  });
  ...
  describe('detectCriticModeFlag', () => {
    ...
    it('rejects a reviewer mode this fork does not ship', () => {
      expect(detectCriticModeFlag('ralph --critic codex fix this')).toBeNull();
      expect(detectCriticModeFlag('ralph --critic=codex fix this')).toBeNull();
    });
  ...
  describe('PRD mode activation in startLoop', () => {
    it('should enable prd_mode when prd.json exists', () => {
      ...
      expect(existsSync(join(testDir, '.lmgh', 'state', 'ultrawork-state.json'))).toBe(false);
    });
  ...
  describe('story-aware architect verification', () => {
    ...
    it('should support critic verification prompts', () => {
      ...
      expect(prompt).toContain('Task(subagent_type="let-me-go-home:critic"');
  ...
  describe('integration: PRD-driven verification', () => {
    ...
    it('stores selected critic mode in Ralph state', () => {
      const hook = createRalphLoopHook(testDir);
      hook.startLoop(undefined, 'Implement caching', { criticMode: 'critic' });

      const state = readRalphState(testDir);
      expect(state?.critic_mode).toBe('critic');
    });
