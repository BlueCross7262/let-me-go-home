...
function openMutationDb(lockPath: string): BetterSqlite3 | null {
  ...
  try {
    ...
    for (const sidecar of [dbPath, `${dbPath}-wal`, `${dbPath}-shm`, `${dbPath}-journal`]) {
      try {
        ...
        if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
          if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] openMutationDb sidecar-stat-error ${sidecar} ${(error as NodeJS.ErrnoException).code}`);
          return null;
        }
      }
    }
    ...
    if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] openMutationDb open/exec failed for ${lockPath}: ${(error as Error)?.message}`);
...
function acquireLockAt(path: string, attempts = 50): MutationLock | null {
  ...
  if (!processStart) {
    try { db.close(); } catch { /* best effort */ }
    if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] acquireLockAt processStart-null ${path}`);
  ...
  try {
    ...
    if (rawRow) {
      ...
      if (!row) { db.exec('ROLLBACK'); db.close(); if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] acquireLockAt row-invalid ${path}`); return null; }
      const live = ownerLive(row);
      if (live === null || live) {
        db.exec('ROLLBACK');
        db.close();
        if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] acquireLockAt row-live=${live} ${path}`);
    ...
    if (artifact !== 'absent') {
      if (!artifact) { db.exec('ROLLBACK'); db.close(); console.error(`${LOCK_LOG_TAG} state_mutation_lock_unverifiable: ${path}`); return null; }
      const live = ownerLive(artifact);
      if (live === null || live) {
        db.exec('ROLLBACK');
        db.close();
        if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] acquireLockAt artifact-live=${live} ${path}`);
        if (live === null || attempts <= 1) return null;
        Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 10);
        return acquireLockAt(path, attempts - 1);
      }
    ...
    if (!publishLockOwner(path, owner)) {
      db.exec('ROLLBACK');
      db.close();
      if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] acquireLockAt publish-failed ${path}`);
    ...
    if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] acquireLockAt caught-error ${path} code=${code} msg=${(error as Error)?.message}`);
...
export function clearStateFileLockedIf(
  ...
  if (process.env.NODE_ENV === 'test' && process.env.LMGH_TEST_CONDITIONAL_CLEAR_REPLACEMENT_PATH === filePath && process.env.LMGH_TEST_CONDITIONAL_CLEAR_REPLACEMENT_BASE64) {
    try {
      const replacement = JSON.parse(Buffer.from(process.env.LMGH_TEST_CONDITIONAL_CLEAR_REPLACEMENT_BASE64, 'base64').toString('utf8')) as Record<string, unknown>;
...
export function writeStateFileLockedIf(
  ...
  if (process.env.NODE_ENV === 'test' && process.env.LMGH_TEST_CONDITIONAL_WRITE_REPLACEMENT_PATH === filePath && process.env.LMGH_TEST_CONDITIONAL_WRITE_REPLACEMENT_BASE64) {
    try {
      const replacement = JSON.parse(Buffer.from(process.env.LMGH_TEST_CONDITIONAL_WRITE_REPLACEMENT_BASE64, 'base64').toString('utf8')) as Record<string, unknown>;
      atomicWriteJsonSync(filePath, replacement);
    } finally {
      delete process.env.LMGH_TEST_CONDITIONAL_WRITE_REPLACEMENT_PATH;
      delete process.env.LMGH_TEST_CONDITIONAL_WRITE_REPLACEMENT_BASE64;
    }
  }
  if (!existsSync(filePath)) return 'skipped';
  const lock = acquireMutationLock(filePath);
...
export function writeStateFileLockedCreateIf(
  ...
  if (!recoverEmergencyStateFile(filePath)) { if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] CreateIf recoverEmergency failed ${filePath}`); return 'failed'; }
  const lock = acquireMutationLock(filePath);
  if (!lock) { if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] CreateIf acquireMutationLock failed ${filePath}`); return 'failed'; }
  try {
    if (process.env.NODE_ENV === 'test' && process.env.LMGH_TEST_CONDITIONAL_CREATE_REPLACEMENT_PATH === filePath && process.env.LMGH_TEST_CONDITIONAL_CREATE_REPLACEMENT_BASE64) {
      try {
        const replacement = JSON.parse(Buffer.from(process.env.LMGH_TEST_CONDITIONAL_CREATE_REPLACEMENT_BASE64, 'base64').toString('utf8')) as Record<string, unknown>;
        atomicWriteJsonSync(filePath, replacement);
      } finally {
    ...
    if (existsSync(filePath)) {
      ...
      catch (error) { if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] CreateIf JSON-parse-failed ${filePath} ${(error as Error)?.message}`); return 'failed'; }
    }
    if (!predicate(current)) return 'skipped';
    atomicWriteJsonSync(filePath, transform(current));
    return 'written';
  } catch (error) {
    if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] CreateIf caught-error ${filePath} ${(error as Error)?.message}`);
    return 'failed';
  } finally {
    releaseMutationLock(lock);
  }
...
function publishEmergencyFileExclusive(path: string, content: string): boolean {
  ...
  try {
    ...
    if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] publishEmergencyFileExclusive failed path=${path} tempPath=${tempPath} pathExists=${existsSync(path)} err=${(error as NodeJS.ErrnoException)?.code} ${(error as Error)?.message}`);
    return false;
  } finally {
...
function acquireRecoveryClaim(path: string, attempts = 50): MutationLockOwner | null {
  ...
  if (!processStart) {
    ...
    if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] acquireRecoveryClaim processStart-null ${path}`);
    if (attempts <= 1) return null;
    Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 10);
    return acquireRecoveryClaim(path, attempts - 1);
  }
  const lock = acquireLockAt(`${path}.recovery.guard`, attempts);
  if (!lock || 'unlocked' in lock) { if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] acquireRecoveryClaim guard-lock-null ${path}`); return null; }
  const existing = readRecoveryClaim(path);
  if (existing) {
    const live = ownerLive(existing);
    if (live === null || live) { releaseMutationLock(lock); if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] acquireRecoveryClaim existing-live=${live} ${path}`); return null; }
    try { unlinkSync(path); } catch (error) { releaseMutationLock(lock); if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] acquireRecoveryClaim existing-unlink-failed ${path} ${(error as NodeJS.ErrnoException).code}`); return null; }
  }
  const owner: MutationLockOwner = { version: 1, pid: process.pid, processStart, createdAt: new Date().toISOString(), nonce: randomUUID() };
  if (!publishEmergencyFileExclusive(path, JSON.stringify(owner))) {
    releaseMutationLock(lock);
    if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] acquireRecoveryClaim publish-failed ${path}`);
    return null;
  }
  return owner;
}
    ...
    if (current && sameRecoveryClaim(current, owner)) {
      ...
      for (let attempt = 0; attempt < 10; attempt += 1) {
        try {
          ...
          if (attempt === 9) {
            if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] releaseRecoveryClaim unlink-failed-after-retries ${path} ${(error as NodeJS.ErrnoException).code}`);
            break;
          }
          Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 10);
        }
...
function replaceGenerationForTest(path: string): void {
  if (
    ...
    process.env.LMGH_TEST_GENERATION_CLEAR_REPLACEMENT_PATH !== path ||
    !process.env.LMGH_TEST_GENERATION_CLEAR_REPLACEMENT_BASE64
  ) return;
  try {
...
function hasUnattributableRecoveryClaimArtifact(filePath: string, recoveryClaim?: MutationLockOwner): boolean {
  ...
  try {
    ...
    if (!existsSync(claimPath)) {
      ...
      if (result && process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] hasUnattributable no-claim-file-but-recoveryClaim-set ${filePath}`);
      return result;
    }
    if (!recoveryClaim) {
    ...
    if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] hasUnattributable caught-error ${filePath} ${(error as Error)?.message}`);
    return true;
  }
}
...
function emergencyReplaceAtRecoveryBoundary(filePath: string): void {
  ...
  try {
    const replacements = JSON.parse(Buffer.from(process.env.LMGH_TEST_EMERGENCY_RECOVERY_REPLACEMENT_BASE64, 'base64').toString('utf8')) as Array<{ path: string; content: string }>;
    const directory = dirname(filePath);
...
export function recoverEmergencyStateFile(filePath: string, options?: EmergencyRecoveryOptions): boolean {
  ...
  if (!existsSync(journalPath)) {
    ...
    if (!claim) { if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] recoverEmergency no-journal-claim-null ${filePath}`); return false; }
    try {
      if (existsSync(journalPath) || !sharedRecoveryArtifactsAuthorized(filePath, authorizeState, claim)) { if (process.env.LMGH_LOCK_DEBUG) console.error(`[lock-debug] recoverEmergency no-journal-revalidate-false ${filePath}`); return false; }
...
function emergencyCrashAt(phase: string): boolean {
  return process.env.NODE_ENV === 'test' && process.env.LMGH_TEST_EMERGENCY_CRASH_PHASE === phase;
}
...
function emergencyReplaceAfterPredicate(filePath: string): void {
  if (process.env.NODE_ENV !== 'test' || process.env.LMGH_TEST_EMERGENCY_REPLACEMENT_PATH !== filePath || !process.env.LMGH_TEST_EMERGENCY_REPLACEMENT_BASE64) return;
  try {
    const replacement = JSON.parse(Buffer.from(process.env.LMGH_TEST_EMERGENCY_REPLACEMENT_BASE64, 'base64').toString('utf8')) as Record<string, unknown>;
    atomicWriteJsonSync(filePath, replacement);
  } finally {
...
function emergencyReplaceAtCaptureBoundary(filePath: string): void {
  ...
  try {
    const replacement = JSON.parse(Buffer.from(process.env.LMGH_TEST_EMERGENCY_CAPTURE_REPLACEMENT_BASE64, 'base64').toString('utf8')) as Record<string, unknown>;
    atomicWriteJsonSync(filePath, replacement);
  } finally {
...
function resolveStateRoot(directory?: string): string {
  ...
  // getLmghRoot() call would reclassify HOME as that repository.
...
function getLegacyStateCandidates(mode: string, directory?: string): string[] {
  ...
  return [
    ...
    join(getLmghRoot(baseDir), `${normalizedName}.json`),
  ];
}

function getRuntimeArtifactCandidates(mode: string, directory?: string, sessionId?: string): string[] {
  const baseDir = resolveStateRoot(directory);
  const stateRoot = join(getLmghRoot(baseDir), 'state');
  const artifactNames = [
 ...
 *
 * Normally the state file lives under `.lmgh/state/sessions/{sessionId}/`.
 ...
 *
 * A fresh `/let-me-go-home:cancel` invocation may run in a new Claude session id while the
 ...
 * (`.lmgh/sessions/{sessionId}.json`) before returning a sibling session's file
...
export function findCompletedSessionStateCandidates(
  ...
  for (const sid of listSessionIds(baseDir)) {
    ...
    const completionEvidencePath = join(getLmghRoot(baseDir), 'sessions', `${sid}.json`);
...
export function writeModeStateIfAbsent(
  ...
  try {
    ...
    else ensureLmghDir('state', baseDir);
