 ...
 *
 * Provides strict path validation and resolution for .lmgh/ paths,
 * ensuring all operations stay within the worktree boundary.
 *
 * Supports LMGH_STATE_DIR environment variable for centralized state storage.
 * When set, state is stored at $LMGH_STATE_DIR/{project-identifier}/ instead
 * of {worktree}/.lmgh/. This preserves state across worktree deletions.
 */
...
import { LOG_TAG, STATE_DIR, WORKSPACE_MARKER, statePath } from '../constants/namespace.js';

/**
 * Workspace marker filename. A directory containing this file is treated as
 * the anchor regardless of git status — enables multi-repo workspaces
 ...
 *
 * Resolution order in getLmghRoot(): LMGH_STATE_DIR > workspace marker > git > cwd.
 */
export { WORKSPACE_MARKER };

/** Standard state-root subdirectories */
export const LmghPaths = {
  ROOT: STATE_DIR,
...
export function findWorkspaceRoot(startDir?: string): string | null {
  ...
  while (true) {
    // Stop before scanning $HOME (or above) so a stray ~/.lmgh-workspace does
 ...
 * `git rev-parse --show-toplevel` stops at the submodule and `.lmgh/` would be
...
function resolveSuperprojectRoot(cwd: string): string | null {
  ...
  for (let depth = 0; depth < 32; depth++) {
    ...
    // git prints POSIX separators even on Windows. The --show-toplevel branch
    // canonicalizes before returning, so leaving this one raw made the two
    // halves of the same function disagree on separators and broke every
    // string comparison against a path built with join() or realpathSync().
 ...
 * Legacy cwd-local `.lmgh/` trees are never adopted implicitly; callers must
 ...
 * working tree so `.lmgh/` state anchors to the monorepo root rather than
...
// ============================================================================
// LMGH_STATE_DIR SUPPORT (Issue #1014)
// ============================================================================
...
/**
 * Scan sibling subdirs of a workspace anchor for pre-existing .lmgh/state/ content.
 ...
 * getLmghRoot() invocation to keep the hot path free of readdirSync calls.
 */
export function warnSiblingRetrofit(workspaceAnchor: string, sessionId?: string): void {
  ...
  const sharedLmgh = join(workspaceAnchor, LmghPaths.ROOT);
  if (sessionId) {
    const markerPath = join(sharedLmgh, 'state', `sibling-retrofit-warned-${sessionId}.json`);
  ...
  for (const entry of entries) {
    ...
    const siblingStateDir = join(workspaceAnchor, entryName, LmghPaths.ROOT, 'state');
    if (existsSync(siblingStateDir)) {
      legacyDirs.push(join(workspaceAnchor, entryName, LmghPaths.ROOT));
    }
  }
  ...
  process.stderr.write(
    `${LOG_TAG} workspace-retrofit warning: ${WORKSPACE_MARKER} anchor found at ${workspaceAnchor}\n` +
    `  but sibling repos have pre-existing local ${LmghPaths.STATE}/ content:\n${dirList}\n` +
    `  Shared state will go to: ${sharedLmgh}\n` +
    `  To migrate legacy state: set LMGH_MIGRATE_LEGACY_STATE=1 and run /let-me-go-home:setup\n` +
    `  Or manually copy state files to ${sharedLmgh}/state/\n`
  );

  // Write disk marker so subsequent hook firings in the same session stay silent
  if (sessionId) {
    try {
      const stateDir = join(sharedLmgh, 'state');
 ...
 * Also removes any disk markers under the given lmghStateDir when provided.
 * @internal
 */
export function clearSiblingRetrofitWarnings(lmghStateDir?: string): void {
  siblingRetrofitWarned.clear();
  if (lmghStateDir) {
    try {
      const stateDir = join(lmghStateDir, 'state');
...
export function getProjectIdentifier(worktreeRoot?: string): string {
  ...
  // project identifier is a state *identity* (used for LMGH_STATE_DIR centralized
  ...
  // test. The #3349 climb applies only to the on-disk `.lmgh/` *location*
  // (getLmghRoot's default branch), not to identity. The no-arg fallback uses
  ...
  // below sees the unclimbed root so an inner `.lmgh-workspace` marker is honored.
...
/**
 * Get the .lmgh root directory path.
 *
 * When LMGH_STATE_DIR is set, returns $LMGH_STATE_DIR/{project-identifier}/
 * instead of {worktree}/.lmgh/. This allows centralized state storage that
 * survives worktree deletion.
 *
 * @param worktreeRoot - Optional worktree root
 * @returns Absolute path to the lmgh root directory
 */
export function getLmghRoot(worktreeRoot?: string): string {
  const customDir = process.env.LMGH_STATE_DIR;
  if (customDir) {
    // Centralized state lives at $LMGH_STATE_DIR/{projectId} — outside the
    // working tree — so the #3349 stray-`.lmgh`-in-submodule problem does not
    ...
    // Log notice if both legacy .lmgh/ and new centralized dir exist
    const legacyPath = join(root, LmghPaths.ROOT);
    const warningKey = `${legacyPath}:${centralizedPath}`;
    if (!dualDirWarnings.has(warningKey) && existsSync(legacyPath) && existsSync(centralizedPath)) {
      dualDirWarnings.add(warningKey);
      console.warn(
        `${LOG_TAG} Both legacy state dir (${legacyPath}) and centralized state dir (${centralizedPath}) exist. ` +
  ...
  // share the same .lmgh/ at the marker location.
  ...
  if (workspaceAnchor && !isSensitiveStateLocation(workspaceAnchor)) {
    ...
    try {
      const legacyPathW = join(workspaceAnchor, LmghPaths.ROOT);
      ...
      if (discoveredCentral) {
        ...
        if (!dualDirWarnings.has(warningKeyW) && existsSync(legacyPathW) && existsSync(centralizedPathW)) {
          ...
          console.warn(
            `${LOG_TAG} Both legacy state dir (${legacyPathW}) and centralized state dir (${centralizedPathW}) exist. ` +
              `Using legacy dir (LMGH_STATE_DIR not set in this process). Set LMGH_STATE_DIR via settings.json env to use centralized dir consistently.`
          );
        }
      }
    } catch { /* best-effort diagnostic only — never break resolution */ }
    return join(workspaceAnchor, LmghPaths.ROOT);
  }

  const root = resolveStateAnchorRoot(worktreeRoot);
  if (!getGitTopLevel(root)) {
    return join(resolveNonGitStateAnchor(root), LmghPaths.ROOT);
  }
...
/**
 * Resolve a relative path under .lmgh/ to an absolute path.
 * Validates the path is within the lmgh boundary.
 *
 * @param relativePath - Path relative to .lmgh/ (e.g., "state/ralph.json")
 * @param worktreeRoot - Optional worktree root (auto-detected if not provided)
 * @returns Absolute path
 * @throws Error if path would escape lmgh boundary
 */
export function resolveLmghPath(relativePath: string, worktreeRoot?: string): string {
  validatePath(relativePath);

  const lmghDir = getLmghRoot(worktreeRoot);
  const fullPath = normalize(resolve(lmghDir, relativePath));

  // Verify resolved path is still under the lmgh directory
  const relativeToLmgh = relative(lmghDir, fullPath);
  if (relativeToLmgh.startsWith('..') || relativeToLmgh.startsWith(sep + '..')) {
...
/**
 * Ensure a directory exists under .lmgh/.
 * Creates parent directories as needed.
 *
 * @param relativePath - Path relative to .lmgh/
 * @param worktreeRoot - Optional worktree root
 * @returns Absolute path to the created directory
 */
export function ensureLmghDir(relativePath: string, worktreeRoot?: string): string {
...
export function getWorktreeNotepadPath(worktreeRoot?: string): string {
  return join(getLmghRoot(worktreeRoot), 'notepad.md');
}
...
export function getWorktreeProjectMemoryPath(worktreeRoot?: string): string {
  return join(getLmghRoot(worktreeRoot), 'project-memory.json');
}
...
export function resolvePlanPath(planName: string, worktreeRoot?: string): string {
  ...
  return join(getLmghRoot(worktreeRoot), 'plans', `${planName}.md`);
}
...
export function resolveResearchPath(name: string, worktreeRoot?: string): string {
  validatePath(name);
  return join(getLmghRoot(worktreeRoot), 'research', name);
}
...
export function resolveLogsPath(worktreeRoot?: string): string {
  return join(getLmghRoot(worktreeRoot), 'logs');
}
...
export function resolveWisdomPath(planName: string, worktreeRoot?: string): string {
  validatePath(planName);
  return join(getLmghRoot(worktreeRoot), 'notepads', planName);
}

/**
 * Check if an absolute path is under the .lmgh directory.
 * @param absolutePath - Absolute path to check
 */
export function isPathUnderLmgh(absolutePath: string, worktreeRoot?: string): boolean {
  const lmghRoot = getLmghRoot(worktreeRoot);
  const normalizedPath = normalize(absolutePath);
  const normalizedLmgh = normalize(lmghRoot);
  return normalizedPath.startsWith(normalizedLmgh + sep) || normalizedPath === normalizedLmgh;
}

/**
 * Ensure all standard .lmgh subdirectories exist.
 */
export function ensureAllLmghDirs(worktreeRoot?: string): void {
  const lmghRoot = getLmghRoot(worktreeRoot);
  const subdirs = ['', 'state', 'plans', 'research', 'logs', 'notepads', 'drafts'];
  for (const subdir of subdirs) {
    const fullPath = subdir ? join(lmghRoot, subdir) : lmghRoot;
    if (!existsSync(fullPath)) {
      try {
        mkdirSync(fullPath, { recursive: true });
      } catch (err) {
...
export function isValidTranscriptPath(transcriptPath: string): boolean {
  ...
  // Allowed: [$CLAUDE_CONFIG_DIR|~/.claude], ~/.lmgh/..., system temp dir
 ...
 * Path: {lmghRoot}/state/sessions/{sessionId}/{mode}-state.json
 *
 * @deprecated Use resolveSessionStatePaths instead.
 ...
 *
 * Fields:
 *  - `sessionScoped`: `.lmgh/state/sessions/{sessionId}/{name}.json` (or empty when no sid).
 *  - `legacy`: `.lmgh/state/{name}.json` — preserved for backwards-compat reads.
 ...
 * fallback, write session-only). When migrate=true OR `LMGH_MIGRATE_LEGACY_STATE=1`
...
export function resolveSessionStatePaths(
  ...
  validateSessionId(sessionId);
  const sessionScoped = resolveLmghPath(`state/sessions/${sessionId}/${normalizedName}.json`, worktreeRoot);
...
export function isLegacyStateMigrationEnabled(): boolean {
  return process.env.LMGH_MIGRATE_LEGACY_STATE === '1';
}

/**
 * Get the session state directory path.
 * Path: {lmghRoot}/state/sessions/{sessionId}/
 *
 * @param sessionId - Session identifier
 * @param worktreeRoot - Optional worktree root
...
export function getSessionStateDir(sessionId: string, worktreeRoot?: string): string {
  validateSessionId(sessionId);
  return join(getLmghRoot(worktreeRoot), 'state', 'sessions', sessionId);
}
...
export function listSessionIds(worktreeRoot?: string): string[] {
  const sessionsDir = join(getLmghRoot(worktreeRoot), 'state', 'sessions');
 ...
 *
 * This ensures .lmgh/ state is always written at the worktree root,
...
export function resolveToWorktreeRoot(directory?: string): string {
  // The resolved root feeds BOTH on-disk `.lmgh/` placement AND, under
  // LMGH_STATE_DIR, the centralized-state *identity* (getProjectIdentifier).
  // The #3349 submodule→superproject climb exists ONLY to place `.lmgh/` at the
  // superproject working tree; it must NOT change a submodule's centralized
  // identity (that contract is documented on getProjectIdentifier/getLmghRoot).
  // So when LMGH_STATE_DIR is set — where on-disk placement is moot and identity
  ...
  const resolveRoot = process.env.LMGH_STATE_DIR ? getGitTopLevel : getWorktreeRoot;
  if (directory) {
 ...
 * This prevents .lmgh/state/ from being created in subdirectories (#576)
...
export function validateWorkingDirectory(workingDirectory?: string): string {
  ...
  // the explicitly requested directory; getLmghRoot() applies the stable
  // non-git anchor and prevents a per-directory .lmgh/ from being created.
