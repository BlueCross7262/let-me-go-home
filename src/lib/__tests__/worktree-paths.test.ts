...
try {
  const probe = join(tmpdir(), `lmgh-symlink-probe-${process.pid}`);
  const probeTarget = join(tmpdir(), `lmgh-symlink-target-${process.pid}`);
...
describe('worktree-paths', () => {
  ...
  describe('resolveLmghPath', () => {
    it('should resolve paths under .lmgh directory', () => {
      const result = resolveLmghPath('state/ralph.json', TEST_DIR);
      expect(result).toBe(join(TEST_DIR, '.lmgh', 'state', 'ralph.json'));
    });

    it('should reject paths that escape .lmgh boundary', () => {
      expect(() => resolveLmghPath('../secret.txt', TEST_DIR)).toThrow('path traversal');
    });
  });
  ...
  describe('resolveStatePath', () => {
    it('should resolve state file paths with -state suffix', () => {
      ...
      expect(result).toBe(join(TEST_DIR, '.lmgh', 'state', 'ralph-state.json'));
    });

    it('should handle input already having -state suffix', () => {
      const result = resolveStatePath('ultrawork-state', TEST_DIR);
      expect(result).toBe(join(TEST_DIR, '.lmgh', 'state', 'ultrawork-state.json'));
    });
  ...
  describe('ensureLmghDir', () => {
    it('should create directories under .lmgh', () => {
      const result = ensureLmghDir('state', TEST_DIR);
      expect(result).toBe(join(TEST_DIR, '.lmgh', 'state'));
  ...
  describe('helper functions', () => {
    it('getWorktreeNotepadPath returns correct path', () => {
      ...
      expect(result).toBe(join(TEST_DIR, '.lmgh', 'notepad.md'));
    });

    it('getWorktreeProjectMemoryPath returns correct path', () => {
      const result = getWorktreeProjectMemoryPath(TEST_DIR);
      expect(result).toBe(join(TEST_DIR, '.lmgh', 'project-memory.json'));
    });

    it('getLmghRoot returns correct path', () => {
      const result = getLmghRoot(TEST_DIR);
      expect(result).toBe(join(TEST_DIR, '.lmgh'));
    });

    it('resolvePlanPath returns correct path', () => {
      const result = resolvePlanPath('my-feature', TEST_DIR);
      expect(result).toBe(join(TEST_DIR, '.lmgh', 'plans', 'my-feature.md'));
    });

    it('resolveResearchPath returns correct path', () => {
      const result = resolveResearchPath('api-research', TEST_DIR);
      expect(result).toBe(join(TEST_DIR, '.lmgh', 'research', 'api-research'));
    });

    it('resolveLogsPath returns correct path', () => {
      const result = resolveLogsPath(TEST_DIR);
      expect(result).toBe(join(TEST_DIR, '.lmgh', 'logs'));
    });

    it('resolveWisdomPath returns correct path', () => {
      const result = resolveWisdomPath('my-plan', TEST_DIR);
      expect(result).toBe(join(TEST_DIR, '.lmgh', 'notepads', 'my-plan'));
    });
  });

  describe('isPathUnderLmgh', () => {
    it('should return true for paths under .lmgh', () => {
      expect(isPathUnderLmgh(join(TEST_DIR, '.lmgh', 'state', 'ralph.json'), TEST_DIR)).toBe(true);
      expect(isPathUnderLmgh(join(TEST_DIR, '.lmgh'), TEST_DIR)).toBe(true);
    });

    it('should return false for paths outside .lmgh', () => {
      expect(isPathUnderLmgh(join(TEST_DIR, 'src', 'file.ts'), TEST_DIR)).toBe(false);
      expect(isPathUnderLmgh('/etc/passwd', TEST_DIR)).toBe(false);
    });
  });

  describe('ensureAllLmghDirs', () => {
    it('should create all standard .lmgh subdirectories', () => {
      ensureAllLmghDirs(TEST_DIR);

      expect(existsSync(join(TEST_DIR, '.lmgh'))).toBe(true);
      expect(existsSync(join(TEST_DIR, '.lmgh', 'state'))).toBe(true);
      expect(existsSync(join(TEST_DIR, '.lmgh', 'plans'))).toBe(true);
      expect(existsSync(join(TEST_DIR, '.lmgh', 'research'))).toBe(true);
      expect(existsSync(join(TEST_DIR, '.lmgh', 'logs'))).toBe(true);
      expect(existsSync(join(TEST_DIR, '.lmgh', 'notepads'))).toBe(true);
      expect(existsSync(join(TEST_DIR, '.lmgh', 'drafts'))).toBe(true);
    });
  });
  ...
  // ==========================================================================
  // LMGH_STATE_DIR TESTS (Issue #1014)
  // ==========================================================================
  ...
  describe('getLmghRoot with LMGH_STATE_DIR (Issue #1014)', () => {
    it('should return default .lmgh path when LMGH_STATE_DIR is not set', () => {
      delete process.env.LMGH_STATE_DIR;
      const result = getLmghRoot(TEST_DIR);
      expect(result).toBe(join(TEST_DIR, '.lmgh'));
    });

    it('should return centralized path when LMGH_STATE_DIR is set', () => {
      const stateDir = mkdtempSync(join(tmpdir(), 'lmgh-state-dir-'));
      try {
        process.env.LMGH_STATE_DIR = stateDir;
        const result = getLmghRoot(TEST_DIR);
        const projectId = 'non-git';
        expect(result).toBe(join(stateDir, projectId));
        expect(result).not.toContain('.lmgh');
      } finally {
    ...
    it('should log warning when both legacy and centralized dirs exist', () => {
      ...
      try {
        ...
        mkdirSync(join(TEST_DIR, '.lmgh'), { recursive: true });
        mkdirSync(join(stateDir, projectId), { recursive: true });

        clearDualDirWarnings();
        getLmghRoot(TEST_DIR);
    ...
    it('should not log warning when only centralized dir exists', () => {
      ...
      try {
        ...
        // Create only centralized dir (no legacy .lmgh/)
        mkdirSync(join(stateDir, projectId), { recursive: true });
  ...
  describe('path functions with LMGH_STATE_DIR', () => {
    let stateDir: string;

    beforeEach(() => {
      stateDir = mkdtempSync(join(tmpdir(), 'lmgh-state-dir-paths-'));
      process.env.LMGH_STATE_DIR = stateDir;
    });
    ...
    it('resolveLmghPath should resolve under centralized dir', () => {
      const result = resolveLmghPath('state/ralph.json', TEST_DIR);
      const projectId = 'non-git';
    ...
    it('isPathUnderLmgh should check against centralized dir', () => {
      const projectId = 'non-git';
      const centralPath = join(stateDir, projectId, 'state', 'ralph.json');
      expect(isPathUnderLmgh(centralPath, TEST_DIR)).toBe(true);

      // Legacy path should NOT be under lmgh when centralized
      expect(isPathUnderLmgh(join(TEST_DIR, '.lmgh', 'state', 'ralph.json'), TEST_DIR)).toBe(false);
    });

    it('ensureAllLmghDirs should create dirs under centralized path', () => {
      ensureAllLmghDirs(TEST_DIR);
      const projectId = 'non-git';
      ...
      // Legacy .lmgh/ should NOT be created
      expect(existsSync(join(TEST_DIR, '.lmgh'))).toBe(false);
    });

    it('ensureLmghDir should create dir under centralized path', () => {
      const result = ensureLmghDir('state', TEST_DIR);
      const projectId = 'non-git';
  ...
  describe('workspace marker (.lmgh-workspace)', () => {
    // Use resolve() so expectations match getLmghRoot's internally-resolved path
    ...
    it('getLmghRoot ignores marker when absent (regression: monorepo flow unchanged)', () => {
      const result = getLmghRoot(workspaceDir);
      expect(result).toBe(join(TEST_DIR, '.lmgh'));
    });

    it('getLmghRoot anchors to marker dir when marker exists in cwd', () => {
      const fs = require('node:fs');
      fs.writeFileSync(join(workspaceDir, '.lmgh-workspace'), '');
      clearWorktreeCache();
      const result = getLmghRoot(workspaceDir);
      expect(result).toBe(join(workspaceDir, '.lmgh'));
    });

    it('getLmghRoot walks up from subdir to find marker', () => {
      const fs = require('node:fs');
      fs.writeFileSync(join(workspaceDir, '.lmgh-workspace'), '');
      clearWorktreeCache();
      const result = getLmghRoot(subrepoDir);
      expect(result).toBe(join(workspaceDir, '.lmgh'));
    });

    it('getLmghRoot prefers marker over a sub-git-repo root', () => {
      const fs = require('node:fs');
      fs.writeFileSync(join(workspaceDir, '.lmgh-workspace'), '');
      try {
    ...
    it('getProjectIdentifier honors explicit id from marker', () => {
      const fs = require('node:fs');
      fs.writeFileSync(
        join(workspaceDir, '.lmgh-workspace'),
  ...
  describe('workspace marker edge cases', () => {
    ...
    beforeEach(() => {
      clearWorktreeCache();
      rootA = resolve(mkdtempSync(join(tmpdir(), 'lmgh-ws-edge-A-')));
    });
    ...
    it('nested markers: inner workspace wins over outer', () => {
      // Structure: rootA/.lmgh-workspace  AND  rootA/B/.lmgh-workspace
      ...
      writeFileSync(join(rootA, '.lmgh-workspace'), '');
      writeFileSync(join(innerB, '.lmgh-workspace'), '');
    ...
    it('LMGH_STATE_DIR overrides workspace marker: getLmghRoot returns centralized path', () => {
      const stateDir = mkdtempSync(join(tmpdir(), 'lmgh-state-override-'));
      try {
        // Drop a workspace marker — without LMGH_STATE_DIR it would steer to rootA/.lmgh
        writeFileSync(join(rootA, '.lmgh-workspace'), '');
        clearWorktreeCache();

        process.env.LMGH_STATE_DIR = stateDir;
        const result = getLmghRoot(rootA);

        // Must use centralized path, not the workspace-anchored .lmgh
        expect(result).not.toBe(join(rootA, '.lmgh'));
    ...
    it('invalid JSON in marker: readWorkspaceMarkerConfig returns {} gracefully', () => {
      writeFileSync(join(rootA, '.lmgh-workspace'), '{ not valid json !!!');
    ...
    it.skipIf(process.platform === 'win32' && !canSymlink)(
      ...
      () => {
        const realDir = mkdtempSync(join(tmpdir(), 'lmgh-ws-real-'));
        const linkDir = join(tmpdir(), `lmgh-ws-link-${process.pid}`);
        try {
          writeFileSync(join(realDir, '.lmgh-workspace'), '');
  ...
  describe('warnSiblingRetrofit + clearSiblingRetrofitWarnings', () => {
    ...
    beforeEach(() => {
      anchorDir = resolve(mkdtempSync(join(tmpdir(), 'lmgh-sibling-anchor-')));
    ...
    it('warns once when siblings have pre-existing .lmgh/state dirs', () => {
      const siblingA = join(anchorDir, 'repoA');
      const siblingB = join(anchorDir, 'repoB');
      mkdirSync(join(siblingA, '.lmgh', 'state'), { recursive: true });
      mkdirSync(join(siblingB, '.lmgh', 'state'), { recursive: true });
      ...
      expect(written).toContain(join(siblingA, '.lmgh'));
      expect(written).toContain(join(siblingB, '.lmgh'));
    });

    it('does not warn when no sibling has .lmgh/state', () => {
      const siblingA = join(anchorDir, 'repoA');
    ...
    it('disk marker is written under {anchor}/.lmgh/state/sibling-retrofit-warned-{sid}.json', () => {
      const siblingA = join(anchorDir, 'repoA');
      mkdirSync(join(siblingA, '.lmgh', 'state'), { recursive: true });
      ...
      const markerPath = join(anchorDir, '.lmgh', 'state', `sibling-retrofit-warned-${sessionId}.json`);
    ...
    it('clearSiblingRetrofitWarnings removes disk markers and allows re-warn', () => {
      ...
      clearSiblingRetrofitWarnings(join(anchorDir, '.lmgh'));
  ...
  describe('resolveSessionStatePaths', () => {
    ...
    beforeEach(() => {
      workDir = resolve(mkdtempSync(join(homedir(), 'lmgh-ssp-')));
      clearWorktreeCache();
    });
    ...
    it('no sessionId: sessionScoped is empty string, effectiveRead and effectiveWrite equal legacy', () => {
      ...
      const expectedLegacy = join(TEST_DIR, '.lmgh', 'state', 'ralph-state.json');
    ...
    it('with sessionId: effectiveWrite is the session-scoped path', () => {
      ...
      const expectedSession = join(TEST_DIR, '.lmgh', 'state', 'sessions', sessionId, 'ultrawork-state.json');
    ...
    it('effectiveRead === sessionScoped after session file is created', () => {
      ...
      const sessionScoped = join(TEST_DIR, '.lmgh', 'state', 'sessions', sessionId, 'ralph-state.json');
      mkdirSync(join(TEST_DIR, '.lmgh', 'state', 'sessions', sessionId), { recursive: true });
  ...
  describe('isLegacyStateMigrationEnabled', () => {
    afterEach(() => {
      delete process.env.LMGH_MIGRATE_LEGACY_STATE;
    });

    it('returns true when LMGH_MIGRATE_LEGACY_STATE=1', () => {
      process.env.LMGH_MIGRATE_LEGACY_STATE = '1';
      expect(isLegacyStateMigrationEnabled()).toBe(true);
    });

    it('returns false when LMGH_MIGRATE_LEGACY_STATE is unset', () => {
      delete process.env.LMGH_MIGRATE_LEGACY_STATE;
      expect(isLegacyStateMigrationEnabled()).toBe(false);
    });

    it('returns false when LMGH_MIGRATE_LEGACY_STATE is set to a non-"1" value', () => {
      process.env.LMGH_MIGRATE_LEGACY_STATE = 'true';
      expect(isLegacyStateMigrationEnabled()).toBe(false);
    });

    it('returns false when LMGH_MIGRATE_LEGACY_STATE is "0"', () => {
      process.env.LMGH_MIGRATE_LEGACY_STATE = '0';
      expect(isLegacyStateMigrationEnabled()).toBe(false);
    });
  });
  ...
  describe('findWorkspaceRoot home-boundary', () => {
    ...
    beforeEach(() => {
      ...
      fakeHome = resolve(mkdtempSync(join(tmpdir(), 'lmgh-fakehome-')));
    ...
    it('marker EXACTLY at home dir is NOT returned (null)', () => {
      writeFileSync(join(fakeHome, '.lmgh-workspace'), '');
      clearWorktreeCache();
    ...
    it('marker BELOW home IS found', () => {
      ...
      writeFileSync(join(projectDir, '.lmgh-workspace'), '');
      clearWorktreeCache();
