...
// atomic-write.ts states the platform split in isPrivateRegularSingleLink:
// the 0o600 mode check is applied only when process.platform is not 'win32',
// because Windows does not carry POSIX permission bits through stat. The two
// assertions below that depend on those bits therefore only hold off Windows.
...
describe('atomicWriteJson', () => {
  ...
  it.each(
    IS_WINDOWS
      ? ['hardlink', 'special', 'replacement']
      : ['hardlink', 'special', 'replacement', 'permissions'],
  )(
