 ...
 *     `.lmgh/` state root.
 *  2. sensitive-location writes: `.lmgh/` was created inside `~/.ssh`,
...
const suiteRoot = mkdtempSync(join(resolve(realTmp), 'lmgh-3873-suite-'));
...
const prevStateDir = process.env.LMGH_STATE_DIR;
const prevDisableMultirepo = process.env.LMGH_DISABLE_MULTIREPO;
...
beforeEach(() => {
  ...
  if (prevDisableMultirepo === undefined) delete process.env.LMGH_DISABLE_MULTIREPO;
  else process.env.LMGH_DISABLE_MULTIREPO = prevDisableMultirepo;
  clearWorktreeCache();
});
...
describe('#3873 non-git state-root anchoring', () => {
  describe('fragmentation collapse (getLmghRoot no-arg)', () => {
    it('three non-git cwds resolve to one canonical root without adopting legacy roots', () => {
      ...
      for (const dir of [base, join(base, 'a'), join(base, 'a', 'b')]) {
        process.chdir(dir);
        clearWorktreeCache();
        roots.add(getLmghRoot());
      }
      expect(roots.size).toBe(1);
      expect([...roots][0]).toBe(join(fakeHome, '.lmgh'));
      ...
      for (const dir of [base, join(base, 'a'), join(base, 'a', 'b')]) {
        process.chdir(dir);
        clearWorktreeCache();
        rootsAfter.add(getLmghRoot());
      }
      expect(rootsAfter.size).toBe(1);
      expect([...rootsAfter][0]).toBe(join(fakeHome, '.lmgh'));
      expect(isDir(join(base, '.lmgh'))).toBe(true);
    });
    ...
    it('nested hook directories do not implicitly adopt an existing project root', () => {
      ...
      mkdirSync(join(project, '.lmgh'), { recursive: true });

      process.chdir(project);
      const projectRoot = getLmghRoot();
      expect(projectRoot).toBe(join(fakeHome, '.lmgh'));
      for (const directory of hookDirs) {
        process.chdir(directory);
        clearWorktreeCache();
        expect(getLmghRoot()).toBe(projectRoot);
      }
    });
  });
  ...
  describe('sensitive-location refusal', () => {
    it('cwd inside ~/.ssh never anchors state in ~/.ssh', () => {
      ...
      mkdirSync(join(nested, '.lmgh'), { recursive: true });
      process.chdir(nested);
      clearWorktreeCache();
      const root = getLmghRoot();
      expect(root).not.toBe(join(nested, '.lmgh'));
      expect(root.startsWith(join(ssh))).toBe(false);
    });

    it('cwd directly in HOME anchors to ~/.lmgh, not HOME itself', () => {
      process.chdir(fakeHome);
      expect(getLmghRoot()).toBe(join(fakeHome, '.lmgh'));
    });

    it('cwd in a user content directory never anchors there', () => {
      const downloads = join(fakeHome, 'Downloads');
      mkdirSync(join(downloads, '.lmgh'), { recursive: true });
      process.chdir(downloads);
      clearWorktreeCache();
      expect(getLmghRoot()).not.toBe(join(downloads, '.lmgh'));
    });
    ...
    it('does not adopt a legacy .lmgh symlink into a sensitive temp descendant', () => {
      ...
      symlinkSync(sensitiveTarget, join(project, '.lmgh'), 'junction');
      process.chdir(project);
      clearWorktreeCache();
      expect(getLmghRoot()).toBe(join(fakeHome, '.lmgh'));
    });

    it('walk-up skips sensitive ancestors without adopting a legacy .lmgh', () => {
  ...
  describe('invariants unchanged', () => {
    it('git repos still anchor at the git toplevel from any subdir', () => {
      ...
      for (const dir of [repo, join(repo, 'x'), join(repo, 'x', 'y')]) {
        process.chdir(dir);
        clearWorktreeCache();
        expect(getLmghRoot()).toBe(join(realpathSync(repo), '.lmgh'));
      }
    });

    it('LMGH_STATE_DIR still centralizes non-git state', () => {
      ...
      expect(root).not.toBe(join(dir, '.lmgh'));
    });

    it('LMGH_STATE_DIR uses one fixed non-git child for multiple directories', () => {
      ...
      process.chdir(first);
      clearWorktreeCache();
      expect(getLmghRoot()).toBe(join(central, 'non-git'));
    ...
    it('.lmgh-workspace remains separate from non-git canonical anchoring', () => {
      const parent = join(scratch, 'ws-parent');
      const inner = join(parent, 'repo-a', 'sub');
      mkdirSync(inner, { recursive: true });
      mkdirSync(join(parent, 'repo-b', '.lmgh'), { recursive: true });
      writeFileSync(join(parent, '.lmgh-workspace'), '{}');
      const anchor = resolveNonGitStateAnchor(inner);
      expect(anchor).not.toBe(join(parent, 'repo-b', '.lmgh'));
      mkdirSync(join(parent, '.lmgh'), { recursive: true });
      clearWorktreeCache();
  ...
  describe('end-to-end state placement', () => {
    it('creates no per-cwd state roots across nested non-git cwds', () => {
      ...
      for (const dir of [join(base, 'one'), join(base, 'one', 'two')]) {
        ...
        clearWorktreeCache();
        ensureAllLmghDirs();
        expect(getLmghRoot()).toBe(root);
      }
      expect(root).toBe(join(fakeHome, '.lmgh'));
      expect(isDir(join(base, '.lmgh'))).toBe(false);
      expect(isDir(join(base, 'one', '.lmgh'))).toBe(false);
      expect(isDir(join(base, 'one', 'two', '.lmgh'))).toBe(false);
    });
  });
});
