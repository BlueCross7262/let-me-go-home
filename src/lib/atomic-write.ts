/**
 * Atomic, durable file writes for let-me-go-home.
...
/**
 * Release the published descriptor so a rollback rename can replace the file.
 *
 * POSIX allows renaming over an open file; Windows does not. Closing here is
 * safe because the caller has already captured the published identity.
 */
function closeDescriptorForRollback(fd: number | null): void {
  if (fd === null) return;
  try {
    fsSync.closeSync(fd);
  } catch {
    // A descriptor that cannot be closed cannot block the rename either.
  }
}
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        // See atomicWriteFileSync: on Windows the rollback rename fails with
        // EPERM while this descriptor is open, and that failure is swallowed.
        try {
          await fd.close();
        } catch {
          // A descriptor that cannot be closed cannot block the rename either.
...
export function atomicWriteFileSync(
  ...
  try {
    ...
    try {
      ...
      // so the rollback below fails with EPERM while the descriptor is live —
      // and rollbackPriorTarget swallows that, leaving the failed generation
      // published. The identity was captured before the hook ran, so the
      // descriptor has no remaining use here.
  ...
  try {
    ...
    for (const write of pending) {
      ...
      try {
        ...
        // See atomicWriteFileSync: on Windows the rollback rename fails with
        // EPERM while this descriptor is open, and that failure is swallowed.
        closeDescriptorForRollback(write.fd);
        write.fd = null;
        rollbackPriorTarget(
          write.path,
          write.backupPath,
