/**
 * Wave G lint test: assert that only the `team` skill writes to `.lmgh/handoffs/`.
 *
 * `.lmgh/handoffs/` is intentionally shared across team runs to enable resume
...
/**
 * Allowed source directories that may reference `.lmgh/handoffs/` as writers.
...
describe('handoffs-writers lint (Wave G)', () => {
  it('no source file outside src/team or src/hooks/team-pipeline references handoffs/', () => {
    ...
    if (violations.length > 0) {
      const msg = [
        '',
        'GOVERNANCE VIOLATION: Only src/team/** and src/hooks/team-pipeline/** may reference .lmgh/handoffs/.',
        'See docs/REFERENCE.md § ".lmgh/handoffs/ shared contract" for the policy.',
        '',
        'Offending lines:',
        ...violations.map(v => `  ${v}`),
        '',
