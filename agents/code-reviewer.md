...
### Good
[CRITICAL] `db.ts:42` 의 SQL 인젝션.
쿼리가 문자열 보간을 쓴다: `SELECT * FROM users WHERE id = ${userId}`.
수정: 파라미터 쿼리를 쓴다: `db.query('SELECT * FROM users WHERE id = $1', [userId])`.

### Good
[CRITICAL] `paginator.ts:42` 의 off-by-one: `for (let i = 0; i <= items.length; i++)` 는 undefined 인 `items[items.length]` 에 접근한다.
수정: `<=` 를 `<` 로 바꾼다.

### Bad
"코드에 몇 가지 문제가 있다. 오류 처리를 개선하고 주석을 좀 추가하는 것을 고려해라."
이 리뷰에는 파일 근거, 심각도, 구체적 수정이 없다.
