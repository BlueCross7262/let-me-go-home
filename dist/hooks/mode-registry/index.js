 ...
 *
 * All modes store state in `.lmgh/state/` subdirectory for consistency.
 */
 ...
 *
 * Empty in this fork: the pair that held this list was autopilot and
 * autoresearch, and neither ships. Ralph and Deep Interview are allowed to
 * overlap — Deep Interview hands off to Ralph, so a handoff turn has both.
 * The mechanism stays so a future exclusive pair has somewhere to go.
 */
...
export function getStateDir(cwd) {
    return join(getLmghRoot(cwd), "state");
}
/**
 ...
 * @deprecated Global state is no longer supported. All modes use local-only state in .lmgh/state/
        ...
        for (const exclusiveMode of EXCLUSIVE_MODES) {
            if (exclusiveMode !== mode &&
                ...
                return {
                    ...
                    message: `Cannot start ${MODE_CONFIGS[mode].name} while ${config.name} is active. Cancel ${config.name} first with /${PLUGIN_NAME}:cancel.`,
                };
            }
        }
    }
 ...
 *
 * Deletes:
 * - Local state file (.lmgh/state/{mode}-state.json)
