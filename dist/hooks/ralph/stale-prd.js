 ...
 * Step 8, session end without `/let-me-go-home:cancel`) leave the divergence
 ...
 *   verification state.
 * - Step 8 (`/let-me-go-home:cancel`): `createRalphLoopHook().cancelLoop()`
 ...
 *   (`.lmgh/state/sessions/{id}/prd.json`) is NOT removed and survives into the
...
export function detectStalePrd(directory, sessionId, options) {
    ...
    // Step 8 (`/let-me-go-home:cancel`) never ran.
...
function getAuditLogPath(directory, sessionId) {
    ...
    const stateDir = resolve(getLmghRoot(directory), 'state');
