...
import { resolveStatePath, ensureLmghDir, resolveStateWorkingDirectory, probeGitTopLevel, resolveSessionStatePath, ensureSessionStateDir, listSessionIds, validateSessionId, getLmghRoot, LmghPaths, } from '../lib/worktree-paths.js';
import { resolveSessionId } from '../lib/session-id.js';
import { validatePayload } from '../lib/payload-limits.js';
import { canClearStateForSession, findCompletedSessionStateFiles, findCompletedSessionStateCandidates, findSessionOwnedStateCandidates, getStateSessionOwner, writeStateFileLocked, writeStateFileLockedCreateIf, clearStateFileLockedIf, } from '../lib/mode-state-io.js';
...
const EXTRA_STATE_ONLY_MODES = ['skill-active'];
...
const STATE_TOOL_MODE_VALUES = [...STATE_TOOL_MODES];
const STATE_WRITE_MODE_VALUES = [...STATE_WRITE_MODES];
const CANCEL_SIGNAL_TTL_MS = 30_000;
const OWNER_SESSION_FALLBACK_MODES = new Set(['ralph']);
const CONVERGED_STATE_PATH_MODES = new Set(['ralph']);
...
const RETIRED_WORKFLOW_MODES = new Set([]);
...
function clearSessionOwnedStateCandidates(mode, root, sessionId, discovered = findSessionOwnedStateCandidates(mode, sessionId, root)) {
    ...
    let hadFailure = false;
    for (const candidate of discovered) {
        const result = clearDiscoveredStateCandidate(candidate, (current) => canClearStateForSession(current, sessionId));
        if (result === 'cleared')
            cleared++;
        else if (result === 'failed')
            hadFailure = true;
...
function discoverAllRootSessionStateCandidates(mode, root) {
    ...
    if (shouldCheckWorkingDirectoryLocalState(root))
        roots.add(getWorkingDirectoryLocalLmghRoot(root));
    ...
    handler: async (args) => {
        ...
        try {
            ...
            if (sessionId) {
                ...
                if (sessionSignalCandidates.length === 0)
