...
function ownerLive(owner) {
    if (process.env.NODE_ENV === 'test' && process.env.LMGH_TEST_EMERGENCY_PROCESS_START_UNKNOWN_PID === String(owner.pid))
        return null;
...
function acquireReclaimMutex(lockPath, processStart, attempts) {
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
        ...
        if (publishLockOwner(mutexPath, owner))
            return owner;
...
function releaseReclaimMutex(lockPath, owner) {
    ...
    try {
        ...
        if (sameOwner(current === 'absent' ? null : current, owner))
            unlinkSync(mutexPath);
    }
    catch { /* best effort */ }
}
...
export function writeStateFileLockedIf(filePath, predicate, transform) {
    ...
    if (process.env.NODE_ENV === 'test' && process.env.LMGH_TEST_CONDITIONAL_WRITE_REPLACEMENT_PATH === filePath && process.env.LMGH_TEST_CONDITIONAL_WRITE_REPLACEMENT_BASE64) {
        try {
            const replacement = JSON.parse(Buffer.from(process.env.LMGH_TEST_CONDITIONAL_WRITE_REPLACEMENT_BASE64, 'base64').toString('utf8'));
            atomicWriteJsonSync(filePath, replacement);
        }
        finally {
...
function replaceGenerationForTest(path) {
    if (process.env.NODE_ENV !== 'test' ||
        ...
        return;
    try {
        const replacement = JSON.parse(Buffer.from(process.env.LMGH_TEST_GENERATION_CLEAR_REPLACEMENT_BASE64, 'base64').toString('utf8'));
...
function hasUnattributableRecoveryClaimArtifact(filePath, recoveryClaim) {
    ...
    try {
        ...
        if (!existsSync(claimPath)) {
            ...
            if (result && process.env.LMGH_LOCK_DEBUG)
...
function emergencyReplaceAtRecoveryBoundary(filePath) {
    if (process.env.NODE_ENV !== 'test' || process.env.LMGH_TEST_EMERGENCY_RECOVERY_REPLACEMENT_PATH !== filePath || !process.env.LMGH_TEST_EMERGENCY_RECOVERY_REPLACEMENT_BASE64)
        return;
    try {
...
function emergencyReplaceAfterPredicate(filePath) {
    if (process.env.NODE_ENV !== 'test' || process.env.LMGH_TEST_EMERGENCY_REPLACEMENT_PATH !== filePath || !process.env.LMGH_TEST_EMERGENCY_REPLACEMENT_BASE64)
        return;
    try {
        const replacement = JSON.parse(Buffer.from(process.env.LMGH_TEST_EMERGENCY_REPLACEMENT_BASE64, 'base64').toString('utf8'));
        atomicWriteJsonSync(filePath, replacement);
    }
    finally {
...
function emergencyReplaceAtCaptureBoundary(filePath) {
    if (process.env.NODE_ENV !== 'test' || process.env.LMGH_TEST_EMERGENCY_CAPTURE_REPLACEMENT_PATH !== filePath || !process.env.LMGH_TEST_EMERGENCY_CAPTURE_REPLACEMENT_BASE64)
        return;
    try {
