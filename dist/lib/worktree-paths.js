...
export function findWorkspaceRoot(startDir) {
    if (process.env.LMGH_DISABLE_MULTIREPO === '1')
        return null;
...
export function isSensitiveStateLocation(dir) {
    ...
    for (;;) {
        ...
        if (home && cursor === candidate && (cursor === home || (process.platform === 'win32' && cursor.toLowerCase() === home.toLowerCase())))
            return true;
        if (name.startsWith('.') && name !== LmghPaths.ROOT)
            return true;
...
export function warnSiblingRetrofit(workspaceAnchor, sessionId) {
    ...
    process.stderr.write(`${LOG_TAG} workspace-retrofit warning: ${WORKSPACE_MARKER} anchor found at ${workspaceAnchor}\n` +
 ...
 * @internal
 */
export function clearSiblingRetrofitWarnings(lmghStateDir) {
 ...
 */
export function getLmghRoot(worktreeRoot) {
    ...
    if (customDir) {
        ...
        if (!dualDirWarnings.has(warningKey) && existsSync(legacyPath) && existsSync(centralizedPath)) {
            ...
            console.warn(`${LOG_TAG} Both legacy state dir (${legacyPath}) and centralized state dir (${centralizedPath}) exist. ` +
        ...
        try {
            ...
            if (discoveredCentral) {
                ...
                if (!dualDirWarnings.has(warningKeyW) && existsSync(legacyPathW) && existsSync(centralizedPathW)) {
                    ...
                    console.warn(`${LOG_TAG} Both legacy state dir (${legacyPathW}) and centralized state dir (${centralizedPathW}) exist. ` +
 ...
 */
export function resolveLmghPath(relativePath, worktreeRoot) {
 ...
 */
export function ensureLmghDir(relativePath, worktreeRoot) {
 ...
 */
export function isPathUnderLmgh(absolutePath, worktreeRoot) {
 ...
 */
export function ensureAllLmghDirs(worktreeRoot) {
    const lmghRoot = getLmghRoot(worktreeRoot);
